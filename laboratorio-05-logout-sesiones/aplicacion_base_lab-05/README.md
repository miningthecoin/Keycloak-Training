...
Estado: laboratorio 05 completado (autenticación OpenID Connect con
Authorization Code + PKCE, cierre de sesión RP-Initiated Logout y
caducidad de sesión por inactividad). Sin roles todavía.
...
└── src/main/
    ...
    │   ├── seguridad/
    │   │   └── SeguridadConfig.java          Reglas de acceso, login OIDC (lab 04) y logout (lab 05)
    │   └── web/
    │       ├── PortalControlador.java        Rutas / y /privada
    │       └── Cliente.java                  Modelo de datos ficticio
    └── resources/
        ├── application.yml                   Puerto 8081, cliente OIDC y caducidad de sesión
        ├── templates/                        Vistas Thymeleaf
        │   ├── fragmentos.html               Cabecera (con "Cerrar sesión") y pie comunes
        │   ├── publica.html
        │   └── privada.html
        └── static/estilos.css
```
